...
class LMSStation:
    def __init__(self, gear_ratio=4, microstep=8, step_delay=0.005, threshold=0.2, el_min=10, el_max=170):
        ...
        self.az_actuator = AzimuthController(
            ...
            arg_microstep=self.microstep
        )
        
        self.servo = Servo(angle=self.elevation)
        
        log("INFO", "STATION", f"LMS Station initialized. Threshold: {self.dist_threshold}m, El Limits: [{self.el_min}, {self.el_max}]")
    ...
    def move_to(self, az_angle, el_angle, delay=None):
        """Moves with safety clamping for elevation."""
